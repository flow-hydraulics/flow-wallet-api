...
export function encrypt(encryptionKey: Buffer, value: string): string {
  const iv = randomBytes(ivSize)

  const cipher = createCipheriv(encryptionAlgo, encryptionKey, iv)
...
export function decrypt(encryptionKey: Buffer, value: string): string {
  const ivHex = value.slice(0, ivSize * 2)
  const ciphertextHex = value.slice(ivSize * 2)

  const iv = Buffer.from(ivHex, "hex")

  const decipher = createDecipheriv(encryptionAlgo, encryptionKey, iv)
